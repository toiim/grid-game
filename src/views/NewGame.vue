...
import { createLevel, type Position } from '@/stores/level'
...
const entitiesInSelectedCells = computed<EntityId[]>(() => {
  const entities: EntityId[] = [];
  selectedCells.value.forEach(position => entities.push(...grid.value[position]))
  return entities
})
...
type LevelDefinition = {
  ...
  gridY: number;
  levelName: 'Forest 01',
  background: 'forest_01.png'
  entities: [

  ]
}

const setupGame = (level, levelDefinition: LevelDefinition) => {
...
<template>
  <div class="layout">
    <div>
      <div class="text">
        <!-- {{ selectedCells }} -->
        <div :class="{ selected: selectedEntity === entityId }" @click="() => { selectedEntity = entityId }"
          class="selectable-entity" v-for="entityId in  entitiesInSelectedCells " :key="entityId">{{
            entityId }}</div>
    ...
    <div class="grid">
      <div :class="{ selected: selectedCells.has(position) }" @click="() => { toggleSelectedCell(position) }"
        v-for="( cell, position ) in  grid " :key="position" :text="cell.size.toString()">
        <div>
          {{ cell.size }}
        </div>
      </div>
